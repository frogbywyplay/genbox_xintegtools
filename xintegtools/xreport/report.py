...
class XReport(object):
    ...
    def __init__(self, root=None, portage_configroot=None, with_deps=False):
    ...
    def _filelist_filter(self, _, dirname, fnames):
        if dirname.startswith(self.root):
            ...
            if not dirname.startswith('/'):
                dirname = '/' + dirname
        if any(filter_.search(dirname) for filter_ in self._dir_filters):
            del fnames[:]
            return
    ...
    def vdb_check(self, info_func=None):
        ...
        for pkg_num, pkg in enumerate(self.vdb):
            ...
            for file_ in pkg.pkgfiles:
                ...
                # collision check
                if file_.type != 'dir':
                    if self.vdb_processed.has_key(file_.name):
    ...
    def find_orphans(self, info_func=None):
        ...
        for file_num, file_ in enumerate(self.filelist):
            ...
            if not self.vdb_processed.has_key(file_):
